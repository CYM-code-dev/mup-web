...
def _volfmt(v):
    """体积显示 (镜像 UI volStr): <0.1 mL → 3 位 (μL 精度); 0.1~1 mL 第三位小数非零 → 3 位, 为 0 → 2 位; ≥1 mL → 2 位."""
    if v < 0.1:
        return f"{v:.3f}"
    if v < 1 and round(v, 3) != round(v, 2):
        return f"{v:.3f}"
    return f"{v:.2f}"
...
def _sci_t(x):
    """正文文本科学计数: 1.34004e-3 → '1.340×10⁻³' (α 保留四位有效数字); ≥0.01 原样 (4 位)。"""
    if x == 0 or abs(x) >= 0.01:
        return f"{x:.4g}"
    e = int(math.floor(math.log10(abs(x))))
    sign = "⁻" if e < 0 else ""
    return f"{x/10**e:.3f}×10{sign}{str(abs(e)).translate(_SUP)}"


def _sci_l(x):
    """LaTeX 科学计数: 1.34004e-3 → '1.340\\times10^{-3}' (α 保留四位有效数字)。"""
    if x == 0 or abs(x) >= 0.01:
        return f"{x:.4g}"
    e = int(math.floor(math.log10(abs(x))))
    return f"{x/10**e:.3f}\\times10^{{{e}}}"
...
def _work_flow_rows_multi(grp, unit, analytes=None, cu="mg/L"):
    ...
    (储备液行留空=标品直用→即证书浓度; 相对式无证书浓度→用链行母液浓度), 不反推; 移取/量器/定容量器每点位首行填、余行留空 (镜像固体合并写法)。"""
    ...
    def _vol(pv):
        try:
            ...
            return ""
        return _volfmt(v) if v else ""

    rows = []
    if direct:
        # 直接稀释: work 链各行即工作液点位 — 每点位×组内各目标物一行
        # (多键为中间液时代遗留 → 与前端一致, 仅取首个含有效行的链)
        _keys = [k for k, st in (work or {}).items() if any(s.get("pip_vol") for s in st or [])]
        for dg_steps in ([work[_keys[0]]] if _keys else list((work or {}).values())):
            for s in dg_steps or []:
                pv = s.get("pip_vol")
                fv_str = s.get("flask_vessel")
                fv = _nominal(fv_str)
                if not (pv and float(pv) > 0):
                    continue
                for gi, a in enumerate(analytes or [{}]):
                    cc = a.get("C_cert")
                    sfl = (grp.get("flasks") or {}).get(a.get("ding_group") or "1") or {}
                    ...
                    rows.append(f"| {mcell} | {_vol(pv) if gi == 0 else ''} | "
                                f"{_strip(s.get('pip_vessel')) if gi == 0 else ''} | "
    ...
    for fd in fd_list:
        if not (fd and fd.get("pip_vol") and float(fd["pip_vol"]) > 0):
            continue
        pv = float(fd["pip_vol"])
        dg = fd.get("dg")
        fv_str = flasks.get(dg)
        fv = _nominal(fv_str)
        wchain = work.get(dg) or []
        ...
        for s in wchain:
            sfv = _nominal(s.get("flask_vessel"))
            tgt = _dilute(s.get("母液浓度(mg/L)"), s.get("pip_vol"), sfv)
...
def _solid_inter_work_rows(grp, group_analytes, cu="mg/L"):
    ...
    # 各目标物中间液移取 (母液浓度按纯度/m_std/储备液容量瓶算); 跳过无有效移取体积者
    entries = []
    for a in group_analytes:
        nm = a.get("name", "")
        fd_raw = feeds.get(nm)
        fd_list = fd_raw if isinstance(fd_raw, list) else ([fd_raw] if fd_raw else [])
        ...
        for fd in fd_list:
            ...
            if pv <= 0:
                continue
            entries.append({"dg": fd.get("dg"), "stock": stock, "pv": pv, "vessel": fd.get("pip_vessel"), "nm": nm})
    # 按中间液分组(dg) 聚合, 保序; 同 dg 定容量器/目标浓度合并
    dgs = []
    for e in entries:
        if e["dg"] not in dgs:
            dgs.append(e["dg"])
    rows = []
    for dg in dgs:
        ge = [e for e in entries if e["dg"] == dg]
        fv_str = flasks.get(dg)
        wchain = work.get(dg) or []
        # 目标浓度(合并) = 该中间液工作液稀释链首行母液浓度 (即中间液浓度, 与工作液链首行衔接)
        inter_tgt = wchain[0].get("母液浓度(mg/L)") if wchain else None
        for i, e in enumerate(ge):
            flask_cell = _strip(fv_str) if i == 0 else ""
            tgt_cell = _conc_fmt(inter_tgt) if i == 0 else ""
            # 母液浓度前置目标物名 (报告稀释流程表无源列, 物名仅在母液浓度格体现)
            stock_cell = f"{e['nm']}（{_conc_fmt(e['stock'])}）" if e.get("nm") else _conc_fmt(e["stock"])
            rows.append(f"| {stock_cell} | {_vol(e['pv'])} | {_strip(e['vessel'])} | "
                        f"{flask_cell} | {tgt_cell} |")
        # 工作液链 (串行稀释): 目标 = 母液×移取/定容
        for s in wchain:
            fv = _nominal(s.get("flask_vessel"))
            tgt = _dilute(s.get("母液浓度(mg/L)"), s.get("pip_vol"), fv)
            rows.append(f"| {_conc_fmt(s.get('母液浓度(mg/L)'))} | {_vol(s.get('pip_vol'))} | "
...
def _stock_solid_detail(method, r, analyte, sec="4.3.1", multi=False):
    ...
    multi=True: 多目标物模式 — 引言泛指所有目标物, 以本物为例展开明细 (其余目标物取值见汇总表)."""
    g = lambda x: f"{x:g}"
    sq = math.sqrt
    ...
    if reags:
        ...
        # 混合试剂: 列具名试剂 (如 "THF、正己烷混合试剂"); 全自定义 → "混合试剂"
        _names = [x.get("name", "") for x in reags if x.get("name")]
        solvent = ("、".join(_names) + "混合试剂") if _names else "混合试剂"
    else:
    ...
    A = L.append
    if multi:
        A("本方法所有目标物均由纯品配置为储备液，因此其不确定度主要有三个来源：标准品纯度产生的不确定度"
    ...
    A("$$ u_{rel}(C_{stock}) = \\sqrt{u_{rel}(c_{s,p})^{2} + u_{rel}(c_{s,m})^{2}"
      " + u_{rel}(c_{s,V})^{2}} $$")
    _lead = f"以{analyte}为例，详细的配置和计算过程如下。" if multi else ""
    A(f"{_lead}储备液的配置流程：称取{g(m_std)}g标准品于{fv}mL容量瓶中，"
...
def _stock_liquid_dilute_detail(method, r, analyte, sec="4.3.1", cert_variants=None, cert_rep_names=None):
    ...
    cert_rep_names: 代表物所在档的目标物名串 → 「以 U=X 的目标物(…)为例」括注。"""
    g = lambda x: f"{x:g}"
    sq = math.sqrt
    env_temp = method.get("env_temp", 20.0)
    dtau = r.get("dtau", 5.0)
    ...
    if pk and pv > 0:
        ...
        _has_pip = False
    fv = int(method["stock_flask_volume"] or 0)             # 0 = 无储备液定容步骤(直接稀释·标品直用)
    _has_flask = fv > 0
    tol_f = GLASS_TOLERANCE[("flask", fv)] if _has_flask else 0
    reags = method.get("stock_reagents")
    if reags:
        alpha_s = sum(x["volume"] * x["alpha"] for x in reags) / sum(x["volume"] for x in reags)
        # 混合试剂: 列具名试剂 (如 "THF、正己烷混合试剂"); 全自定义 → "混合试剂"
    ...
    v_ml = tol_f / (sq(6) * fv) if _has_flask else 0      # 定容 容量瓶允差
    v_t = alpha_s * dtau / sq(3)                          # 定容 温度 (定容试剂α)
    u_sv = sq(v_ml ** 2 + v_t ** 2) if _has_flask else 0  # 定容 合成
    L = []
    A = L.append
    if _has_pip:
        ...
        # 直接稀释·标品直用: 无储备液配制步骤, 工作液各点位由买来的高浓液标直接移取定容
        A(f"{analyte}工作液各点位直接由买来的高浓液标准品移取定容制得，无单独的储备液配制步骤，"
          f"其相对不确定度唯一来源于标准品证书浓度 $u_{{rel}}(c_{{s,c}})$。")
        A("$$ u_{rel}(C_{stock}) = u_{rel}(c_{s,c}) $$")
    # 4.3.1.1 证书浓度
    ...
    if cert_mode == "absolute":
        ...
        if cert_variants:
            A(f"标准品证书给出相对扩展不确定度 $U_{{rel}}$（各目标物不尽相同，包含因子 k={kc}）。"
              f"以 $U_{{rel}}$={g(Urel)}% 的目标物{_rep_paren}为例，换算为相对标准不确定度：")
        else:
    ...
    # 4.3.1.{2|3} 定容 (容量瓶允差 + 定容试剂温度) — 复用纯品称量 4.3.1.3 写法; 无定容步骤(直接稀释·标品直用)则整节约去
    if _has_flask:
    ...
    if _has_pip:
        ...
        A("$$ u_{rel}(C_{stock}) = u_{rel}(c_{s,c}) = " + _f(u_cert)
          + " = " + _f(r["u_stock"]) + " $$")
    return L
...
def render(method, r):
    ...
    if wd:
        ...
        for d in wd:
            ...
            _vu = _volfmt(d["v_used"])
            A(f"| {d['nominal']:g}mL{KIND_CN.get(d['kind'], d['kind'])} | {_vu} | {d['n']} | "
...
def _pip_kind_from_label(label):
    """量器选项串 → (kind, 标称mL)。现版 UI 的 flasks[dg] 只写 pip_vessel (pip_kind 恒 null),
    按串名判型/取前导数 — 与 engine_single._vessel_opts 生成的 (kind, 满量程) 一致。"""
    if not label:
        return (None, None)
    s = str(label)
    kind = ("pip_p" if "移液枪" in s else "pip_s" if "单标吸量管" in s else
            "pip_g" if "分度吸量管" in s else "cylinder" if "量筒" in s else None)
    try:
        nom = float(s.split(" mL")[0])
    except ValueError:
        nom = None
    return (kind, nom)
...
def _stock_method_for_analyte(method, grp, a):
    ...
    if grp.get("kind") == "liquid":
        ...
        _pk, _pnom = _pip_kind_from_label(fl.get("pip_vessel"))   # UI 只写 pip_vessel → 串名判型 (legacy pip_kind 优先)
        m.update({
            ...
            "pip_kind": fl.get("pip_kind") or _pk or "",
            "pip_vol": fl.get("pip_vol") or 0,
            "pip_nominal": fl.get("pip_nominal") or _pnom,
...
def render_multi(method, groups, analytes, results):
    ...
    for gi, grp in enumerate(groups):
        ...
        if kind == "solid":
            ...
            _rep_names = ("、".join(_cg[_co[0]]["names"]) if _cvar else None)
            lines.extend(_stock_liquid_dilute_detail(_mr, results[_ri], grp["name"], sec=f"4.3.{_ns}",
                                                     cert_variants=_cvar, cert_rep_names=_rep_names))
        ...
        if _flow:
            A("工作液各点位由买来的高浓标准品（储备液）直接稀释制得（非逐级），各点位母液浓度、移取体积与量器、"
              ...
              "中间液及工作液由标准储备液经多级稀释制得，各级稀释的母液浓度、移取体积与量器、定容量器及目标浓度见下表：")
            nt = _tab()
            ...
            A("")
        if wd:
            # 共用量器 vs 各目标物中间液移取: 收集全组 work_detail; 在所有目标物中均出现且全等→共用,
            # 否则→该目标物中间液移取(按目标物分行, 同器同体积者合并列名)。
            def _dkey(d):
                return (d["kind"], d["nominal"], round(d["v_used"], 6), d["n"], d["tol"])
            _wd_all = [results[i].get("work_detail") or [] for i in idxs]
            _nA = len(_wd_all)
            _with = {}                      # key → 出现该 key 的目标物数
            for _wdi in _wd_all:
                for _k in {_dkey(d) for d in _wdi}:
                    _with[_k] = _with.get(_k, 0) + 1
            _can_split = _nA > 0 and all(_wd_all)
            _shared = [d for d in wd if _with.get(_dkey(d), 0) == _nA] if _can_split else wd
            _pa = {}                        # (kind, nominal, v_used) → {names, d}
            if _can_split:
                for i in idxs:
                    _nm = analytes[i].get("name", "")
                    for d in (results[i].get("work_detail") or []):
                        if _with.get(_dkey(d), 0) < _nA:
                            _g = (d["kind"], d["nominal"], round(d["v_used"], 6))
                            _pa.setdefault(_g, {"names": [], "d": d})
                            _pa[_g]["names"].append(_nm)
            def _vrow(d, lead=None):
                _ksym = "\\sqrt{3}" if d["kind"] in _GRADUATED else "\\sqrt{6}"
                _cells = [f"{d['nominal']:g}mL{KIND_CN.get(d['kind'], d['kind'])}", _volfmt(d["v_used"]),
                          str(d["n"]), f"{d['tol']:g}", f"${_ksym}$",
                          _f(d["urel_ml"]), _f(d["urel_t"]), _f(d["urel"])]
                if lead is not None:
                    _cells = [lead] + _cells
                return "| " + " | ".join(_cells) + " |"
            if _pa:
                A("每一稀释级的移取(吸量管)与定容(容量瓶)各引入体积不确定度，同器同体积多次使用者按使用次数 n 合成。"
                  "考虑量器允差与温度变化(均匀分布√3)，各量器一次使用的相对不确定度如下表。"
                  "中间液各目标物移取量器/体积不同者按目标物分行，工作液量器为各目标物共用：")
                nt = _tab()
                A(f"表{nt} 「{grp['name']}」中间液及工作液稀释量器相对不确定度明细\n")
                A("| 目标物 | 量器 | 使用规格V/mL | 次数n | 允差d/mL | 包含因子k | $u_{rel}(V_容)$ | $u_{rel}(V_τ)$ | $u_{rel}(V)$ |")
                A("|---|---|---|---|---|---|---|---|---|")
                for _v in _pa.values():
                    A(_vrow(_v["d"], "、".join(_v["names"])))
                for d in _shared:
                    A(_vrow(d, "共用"))
            else:
                A("每一稀释级的移取(吸量管)与定容(容量瓶)各引入体积不确定度，"
            ...
            A("其中允差按量器类型取分布（容量瓶/单标线吸量管三角分布√6，分度吸量管均匀分布√3）；"
              ...
              "移液枪允差为移取体积的百分比、按≥V 的校准点取值，按均匀分布√3计入。"
              "单器一次使用 $u_{rel}(V)=\\sqrt{u_{rel}(V_容)^{2}+u_{rel}(V_\\tau)^{2}}$。")
            # 单器一次使用 u_rel(V) 计算示例: 取明细中影响最大的量器展开 允差项+温度项
            if wd:
                ...
                _exnfs = (f"（非满刻度：允差按标称{_exv['nominal']:g}mL查表、体积按实际{_volfmt(_exv['v_used'])}mL计）"
                          if _exv["kind"] == "pip_g" and abs(_exv["v_used"] - _exv["nominal"]) > 1e-9 else "")
                _al = _sci_l(rg.get("work_alpha", rg.get("alpha", 1.19e-3)))
                _dt = rg.get("dtau", 5)
                _vn = f"{_exv['nominal']:g}mL{KIND_CN.get(_exv['kind'], _exv['kind'])}"
                _tol = f"{_exv['tol']:g}"; _vu = _volfmt(_exv["v_used"])
                _uml = _f(_exv["urel_ml"]); _ut = _f(_exv["urel_t"]); _uv = _f(_exv["urel"])
                # 示例量器归属 → 单个目标物名 (逐物量器取首个; 共用取代表物), 余者见上表
                _exg = (_exv["kind"], _exv["nominal"], round(_exv["v_used"], 6))
                if _pa and _exg in _pa:
                    _exwho = _pa[_exg]["names"][0]
                else:
                    _exwho = analytes[idxs[0]].get("name", "") or "各目标物"
                A(f"以{_exwho}使用的{_vn}（{_exrole}{_vu}mL）为例{_exnfs}：")
                A("$$ u_{rel}(V_容) = \\frac{d}{k\\cdot V} = \\frac{" + _tol + "}{" + _exksym
                  + "\\times " + _vu + "} = " + _uml + " $$")
                A("$$ u_{rel}(V_\\tau) = \\frac{\\alpha\\cdot\\Delta\\tau}{\\sqrt{3}} = \\frac{" + _al
                  + "\\times " + str(_dt) + "}{\\sqrt{3}} = " + _ut + " $$")
                A("$$ u_{rel}(V) = \\sqrt{u_{rel}(V_容)^{2}+u_{rel}(V_\\tau)^{2}} = \\sqrt{" + _uml
                  + "^{2}+" + _ut + "^{2}} = " + _uv + " $$")
            # 单目标物方和根算例: 该物中间液+工作液各量器 (n·u²) 求和 → u_rel(C_work); 无明细→False
            def _worked(res, name):
                ...
                for d in exd:
                    ...
                    _uses.append(f"{d['n']}次{d['nominal']:g}mL{KIND_CN.get(d['kind'], d['kind'])}"
                                 f"（{_role}{_volfmt(d['v_used'])}mL）")
                _proc_w = "工作液直接稀释" if (grp.get("inter") or {}).get("direct") else "中间液及工作液稀释"
                A(f"以{name}为例：{_proc_w}过程中使用了" + "、".join(_uses)
                  + "，各量器引入的相对不确定度按方和根合成：")
                if len(_rows) > 4:
            ...
            A("多级稀释按方和根合成：")
            if _same:
                # 全组 u_work 一致 → 以代表物展开算例, 注明各目标物同值
                if not _worked(results[_ex], analytes[_ex].get("name", "")):
                    A(f"$$ u_{{rel}}(C_{{work}}) = \\sqrt{{\\sum n_i \\cdot u_{{rel}}(V_i)^{{2}}}} = {_f(rg['u_work'])} $$")
                else:
                    A(f"各目标物 $u_{{rel}}(C_{{work}})$ 均为 {_f(rg['u_work'])}。")
            else:
                A("$$ u_{rel}(C_{work}) = \\sqrt{\\sum n_i \\cdot u_{rel}(V_i)^{2}} $$")
                A("中间液各目标物移取体积不同，$u_{rel}(C_{work})$ 逐目标物取值：")
                _worked(results[_ex], analytes[_ex].get("name", ""))
                nt = _tab()
    ...
    if _rows44:
        ...
        _is_groups, _is_idx = [], {}
        _is_targets = set()
        for _ti, (_a, _r, _p) in enumerate(_rows44):
            _xk = tuple(x for x, _ in _p)
            if _xk not in _x_idx:
                _x_idx[_xk] = len(_x_groups); _x_groups.append([_xk, []])
            _x_groups[_x_idx[_xk]][1].append(_ti)
            if _a.get("curve_method") == "内标法" and (_a.get("analyte_areas")) and (_a.get("is_areas")):
                _is_targets.add(_ti)
                _ik = tuple(_a.get("is_areas"))
                if _ik not in _is_idx:
                    _is_idx[_ik] = len(_is_groups); _is_groups.append([_ik, []])
                _is_groups[_is_idx[_ik]][1].append(_ti)
        _tcode = {ti: f"T{n}" for n, ti in enumerate(sorted(_is_targets), 1)} if len(_is_groups) > 1 else {}
        def _nm_of(ti):
            n = _rows44[ti][0].get("name", "")
            return f"{n}（{_tcode[ti]}）" if ti in _tcode else n
        # 按浓度分组输出: 每组一行 X (单物=物名; 多物共用=各目标物/代号), 后接组内各物 Ax
        for _xk, _tis in _x_groups:
            if len(_tis) == 1:
                _xlab = _nm_of(_tis[0])
            ...
            for _ti in _tis:
                ...
                if is_is and a_areas and i_areas:    # 内标法: 目标物峰面积逐物; 内标峰面积分组→末尾合并
                    A(f"| {_nm_of(_ti)} | $A_x$ | " + " | ".join(_yfmt(v) for v in a_areas) + " |")
                else:
                    ylab = "响应比" if is_is else "$A_x$"   # 外标法峰面积=分析物峰面积 A_x (与内标法 A_x 行一致)
                    A(f"| {_nm_of(_ti)} | {ylab} | " + " | ".join(_yfmt(y) for _, y in pts) + " |")
        # 内标峰面积: 每组末尾一行 (单组=全体共用; 多组=按目标物代号 T1,T2… 标注)
        for _areas, _tis in _is_groups:
            _vals = [_yfmt(v) for v in _areas] + [""] * (_max_pt - len(_areas))
            _lab = "内标峰面积" if len(_is_groups) == 1 \
                else "内标峰面积（" + ",".join(_tcode[ti] for ti in _tis) + "）"
            A(f"| {_lab} | $A_{{IS}}$ | " + " | ".join(_vals) + " |")
        A("")
    # 方法学叙述 (写一次): 定义 x/y + 线性方程; Sxx/s/u(Q)/u_rel(Q) 各独占一显示行 (对齐单目标物范本,
    # 多目标物无逐物数值, 仅符号式)
    A(f"本方法采用{method_word}定量，最小二乘法拟合出的标准曲线线性方程为 {eq_str}，"
      f"其中 x 为{x_word}，y 为{y_word}"
    ...
    # 4.6 段落: 各物浓度/体积一致→均匀表述(不逐物); 浓度不同(体积同)→逐物列浓度; 体积不同→逐物(浓度+体积)
    _spiked = [a for a in analytes if a.get("spike_std_conc")]
    def _vol_str(a):
        v = a.get("spike_add_vol")
        return "" if not v else (f"{v / 1000:.2f}mL" if v >= 1000 else f"{g(v)}μL")
    ...
    # urel(C) 各目标物显示一致 → 并入"各目标物相同"叙述、移出表格 (与 urel(m)/urel(V) 同处理)
    _C_disps = [(lambda c: _f(c) if c is not None else None)((r.get("components") or {}).get("C"))
                for r in results]
    _same_C = bool(results) and all(d is not None for d in _C_disps) and len(set(_C_disps)) == 1
    # urel(C) 非方法共享项(各物分别由储备液+稀释链评定), 仅在此例各物结果相同时一并列出 → 独句说明
    _C_clause = (f"本例各目标物标准溶液浓度引入的 $u_{{rel}}(C)$ 评定结果相同：$u_{{rel}}(C)={_C_disps[0]}$。"
                 if _same_C else "")
    ...
    _comp_cols = ([("C", "$u_{rel}(C)$")] if not _same_C else []) + \
                 [("Q", "$u_{rel}(Q)$"), ("f", "$u_{rel}(f)$"), ("R", "$u_{rel}(R)$")]
    _ncols = 1 + len(_comp_cols) + 5   # 目标物 + 分量列 + urel(W)/w/U/U%/主导
    A("| 目标物 | " + " | ".join(lbl for _, lbl in _comp_cols) +
      " | $u_{rel}(W)$ | w | U (k=2) | U/w (%) | 主导 |")
    A("|" + "---|" * _ncols)
    for a, r in zip(analytes, results):
        comps = r["components"]
        cells = [_f(comps[k]) if k in comps else "—" for k, _ in _comp_cols]
